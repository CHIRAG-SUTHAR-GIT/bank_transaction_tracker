...
def process_excel_file(filepath):
    """Process Excel file"""
    global df_main, df_other_sheets
    ...
    try:
        ...
        for sheet in xl.sheet_names:
            ...
            # Determine amount column: Column I (index 8) for Cheque, Column F (index 5) for others
        ...
        return True, f"Processed successfully. {len(df_other_sheets)} additional sheets found."
        
    except Exception as e:
        return False, f"Error: {str(e)}"

def get_transaction_breakdown(trans_id):
    """Get breakdown from other sheets - ONLY NON-ZERO amounts"""
    breakdown = []
    ...
    for sheet_name, sheet_info in df_other_sheets.items():
        ...
        # Transaction ID in column D (index 3)
        if len(df.columns) > 3:
            matches = df[df.iloc[:, 3].astype(str).str.strip() == str(trans_id).strip()]
...
def calculate_status(child_trans_id, disputed_amount, current_layer):
    ...
    # Check for children: Parent Trans ID (col D, index 3) matches this Child Trans ID
    # AND layer is higher than current
    children = df_main[
        (df_main.iloc[:, 3].astype(str).str.strip() == str(child_trans_id).strip()) & 
        (df_main.iloc[:, 5] > current_layer)
    ]
    has_children = len(children) > 0
    
    # Get breakdown from other sheets (only non-zero)
...
def get_layer_transactions(layer=1, parent_trans_id=None):
    ...
    for _, row in filtered.iterrows():
        ...
        children = df_main[
            (df_main.iloc[:, 3].astype(str).str.strip() == child_trans_id) & 
            (df_main.iloc[:, 5] > current_layer)
        ]
        if len(children) > 0:
...
def build_hierarchical_data(parent_id=None, layer=1, parent_path="", level=0, visited=None):
    """Build hierarchical data for Excel"""
    if visited is None:
        visited = set()
    ...
    if layer == 1:
        ...
        parent_rows = df_main[df_main.iloc[:, 9].astype(str).str.strip() == str(parent_id).strip()]
        if len(parent_rows) == 0:
            return []
        parent_layer = int(parent_rows.iloc[0, 5])
        
        filtered = df_main[
            (df_main.iloc[:, 3].astype(str).str.strip() == str(parent_id).strip()) & 
            (df_main.iloc[:, 5] > parent_layer)
        ]
